...
class Utils
{
    public static function apiClient()
    {
        ...
        if($appConfig['use_api_client'] == "1") $apiClient = ClientController::initClient();

        return $apiClient;
    }

    public static function apiClientSoap()
    {
        $apiClientSoap = null;
        $appConfig = \parse_ini_file(ROOT . 'config/app.config.ini');
        if($appConfig['use_api_client'] == "1") $apiClientSoap = ClientSoapController::initClientSoap();

        return $apiClientSoap;
    }
    ...
    {
        return ($url != "") ? ((is_file(ROOT.$url)) ? \unlink(ROOT.$url) : exec("rm -rf ".ROOT.$url)) : false;
    }
     ...
     */
    public static function cryptString($string, $key)
    {
     ...
     */
    public static function decryptString($crypt, $key)
    {
